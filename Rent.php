<?php
	...
	$sql = "SELECT Name, Rating, Rent FROM Apt_complex WHERE  (lat - $lat) < 0.03 AND  (lat - $lat > -0.3) AND (lng - $lng) < 0.3 AND  (lng - $lng > -0.3)  ORDER BY Rent";
	    // get result
...
<?php 
	for($count = 0; $count < count($apt); $count++)
	{
		if(isset($_POST[$count]))
		{
			...
			$name = $apt['Name'];
			$A_id = "SELECT A_id FROM Apt_complex WHERE Name = '$name' AND (lat - $lat) < 0.03 AND  (lat - $lat > -0.3) AND (lng - $lng) < 0.3 AND  (lng - $lng > -0.3);";
